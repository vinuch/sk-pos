...
// import { Database } from './supabase'; // Adjust the import path as needed
...
export type OrderItemUpdate = Database['public']['Tables']['OrderItems']['Update'];
...
export function NewOrder({ menuItems, swallows, proteins }: NewOrderProps) {
  ...
  function addToOrder(itemId: number, itemName: string, itemPrice: number) {
    ...
    setOrderItems([
      ...orderItems,
      {
        id: generateRandomId(1000000000, 9999999999),
        menu_item_id: itemId,
        name: itemName,
        swallowQty: 1,
        proteinQty: 1,
        ...
        prevItems.map((item) =>
          item.id === itemId
            ? {
                ...item,
                swallow: swallowId,
                swallowName: swallowName,
                swallow_price: swallowPrice,
              }
            : item
        )
      );
  ...
  function incrementSwallow(itemId?: number) {
    const exists = orderItems.find((orderItem) => orderItem.id === itemId);
    console.log(exists)
      ...
      setOrderItems((prevItems) =>
        prevItems.map((item) =>
          item.id === itemId
            ? {
                ...item,
                swallowQty: (item.swallowQty ?? 0) + 1,
                total:
                (item.swallowQty ?? 0) >= 1
                    ?  (item.total && exists.swallow_price )&& item.total + exists.swallow_price
                    : item.total,
              }
            : item
        )
  ...
  function decrementSwallow(itemId?: number) {
    const exists = orderItems.find((orderItem) => orderItem.id === itemId);

    if (exists && exists.swallowQty && exists.swallowQty > 0) {
      setOrderItems((prevItems) =>
        prevItems.map((item) =>
          item.id === itemId
            ? {
  ...
  function incrementProtein(itemId?: number) {
    const exists = orderItems.find((orderItem) => orderItem.id === itemId);
        ...
        prevItems.map((item) =>
          item.id === itemId
            ? {
                ...
                total:
                (item.proteinQty && item.proteinPerServing) && item.proteinQty >= item.proteinPerServing
                    ? (item.total && exists.proteinPrice) && item.total + exists.proteinPrice
                    : item.total,
              }
            : item
        )
  ...
  function decrementProtein(itemId?: number) {
    const exists = orderItems.find((orderItem) => orderItem.id === itemId);

    if ((exists && exists.proteinQty) && exists.proteinQty > 0) {
      setOrderItems((prevItems) =>
        prevItems.map((item) =>
          item.id === itemId
            ? {
  ...
  async function handleCreateOrder() {
    ...
    let order_summary = orderItems.map(
      (item) =>
        `${item.name} ${
    ...
    await addNewOrder(order_summary, total, orderItems);
      ...
      <div className="flex flex-wrap justify-around gap-3 my-4">
        {menuItems?.map((item) => (
          <div
            ...
            className={`p-4 rounded-md border border-gray my-2 w-full md:w-48p cursor-pointer relative ${
              orderItems.find((orderItem) => orderItem.menu_item_id === item.id)
                ? "bg-slate-100"
                : ""
            }`}
          >
            <span>{item.name}</span>
            {orderItems.find((orderItem) => orderItem.menu_item_id == item.id) ? (
              <span className="text-white bg-red-500 rounded-full inline-flex h-5 w-5  items-center justify-center text-sm absolute right-5 -top-1">
                {
                  orderItems.filter((orderItem) => orderItem.menu_item_id == item.id)
                    .length
                }
              </span>
            ) : null}
            ...
            <Accordion type="single" collapsible className="md:w-6/12 w-full">
              {orderItems?.map((item, idx) => (
                <AccordionItem key={item.id} value={item.name!}>
                  <AccordionTrigger>
                    {" "}
                    <div className=" w-full flex items-center justify-between p-4 rounded-md pr-2 my-2 ">
                      <div className="text-left">
                        ...
                        <span className="font-normal">{` ${
                          item.swallow
                            ? "(" +
                              `${item.swallowQty && item.swallowQty > 1 ? item.swallowQty : ""} ` +
                              item.swallowName +
                              ")"
                            : ""
                        } `}</span>
                        {item.swallow && item.protein ? (
                          <span>{` & `}</span>
                        ) : (
                          ""
                        )}
                        <span className="font-normal">{`${
                          item.protein
                            ? "(" +
                              `${item.proteinQty && item.proteinQty > 1 ? item.proteinQty : ""} ` +
                              item.proteinName +
                              ")"
                            : ""
                        } `}</span>
                  ...
                  <AccordionContent>
                    ...
                    <div className="flex p-2 justify-between items-center">
                      ...
                      <div className="flex gap-2">
                        <button
                          disabled={!item.swallow}
                          className={`${
                            !item.swallow ? "cursor-not-allowed" : ""
                          }`}
                          onClick={(e) => {
                            e.stopPropagation();
                            decrementSwallow(item.id);
                          }}
                        >
                          <FaMinus />
                        </button>
                    ...
                    <div className="flex p-2 justify-between items-center">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`${
                            !item.protein ? "cursor-not-allowed" : ""
                          }`}
                          onClick={(e) => {
                            e.stopPropagation();
                            decrementProtein(item.id);
                          }}
                        >
                          <FaMinus />
                        </button>
